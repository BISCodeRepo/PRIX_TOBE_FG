...
public interface ModificationMapper {
    ...
    @Select("SELECT name, mass_diff, residue, position FROM px_modification WHERE id IN (SELECT mod_id FROM px_user_modification WHERE user_id = #{userId} AND variable = #{variable} AND engine = #{engine})")
    ...
    @Select("SELECT mass_diff FROM px_modification WHERE name = #{name}")
